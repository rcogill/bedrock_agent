...
class BedrockAgent:
    ...
    def _add_cache_point(
        self, msg: ToolMessage | HumanMessage
    ) -> ToolMessage | HumanMessage:
        """
        Add a cache point to a given message.
        """
    ...
    def chat(
        ...
        Args:
            ...
            on_tool_call: Optional callback function that receives 
                          (tool_name, tool_args, tool_result) for each tool invocation.
                          Useful for displaying tool usage to the user.

        Returns:
            The agent's final text response.
        """
        # Add user message to history. Cache system prompt and first message if caching is enabled.
        if self.config.cache_enabled:
            self.conversation_history.append(
                self._add_cache_point(HumanMessage(content=user_input))
            )
        else:
        ...
        while response.tool_calls:
            ...
            # Apply caching if we have accumulated enough new input tokens
            if (
                self.config.cache_enabled
                and response.usage_metadata["input_tokens"]
                > self.config.cache_tokens
            ):
                last_msg = self.conversation_history.pop()
                self.conversation_history.append(self._add_cache_point(last_msg))

            # Get next response
            messages = (
                [SystemMessage(content=self.config.system_prompt)] + self.conversation_history
            )
            response = self.llm_with_tools.invoke(messages)
        ...
        if (
            ...
            "text" not in [x["type"] for x in response.content]
        ):
